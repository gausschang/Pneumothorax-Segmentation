...
cls_weights = [f'../ntuh_classification/model_weights/fold_{i}_eff_cls_ntuh/fold_{i}_eff_cls_ntuh_' for i in range(5)]
seg_weights = [f'../ntuh_segmentation/model_weights/fold_{i}_eff_seg_ntuh/fold_{i}_eff_seg_ntuh_' for i in range(5)]
...
for fold_num in range(5):
    cls_config = {'encoder_name':'tu-tf_efficientnetv2_s_in21ft1k', 'method': 'Unet', 'classes':2,
        'aux_params':{
                    'classes':1,
                    'pooling':'avg',
                    'dropout':0.2,
        ...
        'weight_list':[cls_weights[fold_num]]}
    seg_config = {'encoder_name':'tu-tf_efficientnetv2_s_in21ft1k', 'method': 'Unet', 'classes':2,
        'weight_list':[seg_weights[fold_num]]}

    y, y_pred, detectable = main(fold_num, cls_config, seg_config, save_dir=f'cls_eff_5fold_result')
    y_out.append(y)
...
auc(out[0,:],out[1,:], size_threshold, save_dir='cls_eff_5fold_result', save_name='5cv_avg')
#with open("result_eff.json", "w") as fp:
