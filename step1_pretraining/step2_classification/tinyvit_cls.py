...
class DecoderBlock(nn.Module):
    def __init__(
        ...
        self.up_bn = nn.BatchNorm2d(in_channels) if up_bn else None
        self.skip_bn = nn.BatchNorm2d(skip_channels) if skip_bn else None


    def forward(self, x, skip=None):

        if self.up_bn is not None:
            x = self.up_bn(x)
...
class PatchMerging(nn.Module): # norm
...
def regroup(x):
    B, L, C = x.shape
    n = int(math.sqrt(L))
    x = x.transpose(1, 2).reshape(B, C, n, n)
    return x
...
class TinyViT(nn.Module):
    ...
    def forward(self, x):
        logit, x = self.forward_features(x)
        y = self.up1(x[-1], x[-3])
        y = self.up2(y, x[-4])
        y = self.up3(y, x[-5])
        y = self.segmentation_head(y)
        logit = self.norm_head(logit)
